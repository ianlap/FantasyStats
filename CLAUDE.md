...
3. **`pipeline/build.py`** → `site/data/<season>.json` per season plus `cumulative.json` (all-time franchise aggregates via `pipeline/cumulative.py`, keyed by ESPN team id) and `index.json` (season manifest the site loads first). The static site (`site/`, vanilla JS SPA with hash routing + hand-rolled SVG charts in `site/js/charts.js`) has a season dropdown; cumulative mode hides the Matchups tab and week-level charts. `tests/test_build.py` and `tests/test_cumulative.py` pin these schemas — update them when changing payloads. A new season appears automatically once its first pull lands in `data/raw/<season>/`.
